...
if (isset($data["submit"]))
    switch ($data["code"])
    {
        case "add_post":
            $post = new Post();
            $post->title = isset($data["title"])?htmlspecialchars(trim($data["title"])):"";
            $post->text = isset($data["text"])?htmlspecialchars(trim($data["text"])):"";
            $tags = explode(" ", $data["tags"]);
            array_walk( $tags, function (&$item){$item = htmlspecialchars(trim($item));});
            $post->tags =($tags)?array_filter($tags, function ($elem) {return (bool)$elem;}):"";
...
<div id='form_block'>
    ...
    <form id='add_post_form' method='POST'>
        ...
        <textarea type='text' name='text' cols='40' rows='10'></textarea>
        <br>
        <br>
